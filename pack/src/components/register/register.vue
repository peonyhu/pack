  ...
  <div class="m-con m-reg" id="m-reg">
    ...
    <div class="m-verify mt60">
      <div class="m-vermsg clearfix">
        ...
        <input type="button" v-show="sendBtn" class="next" @click="sendCode" v-model="msgIndentiy"/>
        <input type="button" class="next" v-show="!sendBtn" v-model='useSecond'>
      ...
      <!--<p class="m-frid"><friendshare class="disshow"></friendshare></p>-->
    </div>
  </div>
</template>
...
//      短信倒计时
      timeCount(){
        this.useSecond = this.countSecond-1;
        this.sendBtn = false;
        this.disabled = true;
        var timer = null;
        clearInterval(timer);
        timer=setInterval(()=>{
          if(this.useSecond>0)
          {
            ...
            this.disabled=false;
            this.sendBtn =true;
            this.useSecond= this.countSecond;
            clearInterval(timer);
          }
        },1000)
      },
      sendCode(){
        ...
        this.$http.post('/redPacket/api.php?action=send',params,{
          emulateJSON:true
        }).then((res) => {
          let data = res.data;
          console.log(data);
